...
def generate_launcher():
    """Write a double-click launcher with the resolved conda path baked in.

    Students then just double-click run_demo_server.bat (Windows) / .sh — no
    command line, no conda knowledge. Regenerated on each setup so the paths
    stay correct.
    """
    if not CONDA:
        print("  (Skipping launcher — no conda found.)")
        return None
    inner = f'"{CONDA}" run --no-capture-output -n {ENV_NAME} python "{SERVER}" --open'
    if os.name == "nt":
        path = os.path.join(HERE, "run_demo_server.bat")
        content = (
            "@echo off\r\n"
            "rem Auto-generated by setup_demo_server.py — double-click to run the demo.\r\n"
            "title MMG Demo Server\r\n"
            f"{inner}\r\n"
            "echo.\r\n"
        ...
        path = os.path.join(HERE, "run_demo_server.sh")
        content = (
            "#!/usr/bin/env bash\n"
            "# Auto-generated by setup_demo_server.py — run the demo server.\n"
            f"{inner}\n"
        )
