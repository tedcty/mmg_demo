...
# SSM demo client settings.
#   show_kinematics_tab — the Kinematics (SC/AC/GH angle sliders) tab is a
#                          debug view for the joint-assembly pipeline, not
#                          meant for the outreach/kiosk audience — the
#                          dashboard's Demo Control tab toggles this instead
#                          of it being a hardcoded build-time constant.
#                          Off by default.
SSM_CONFIG_FILE = os.path.join(BASE_DIR, 'ssm_config.json')
SSM_CONFIG_DEFAULT = {'show_kinematics_tab': False}
_ssm_config_lock = threading.Lock()
...
def _load_ssm_config():
    """Read SSM demo client settings from disk; tolerate a missing/corrupt file."""
    try:
        with open(SSM_CONFIG_FILE, encoding='utf-8') as f:
            data = json.load(f)
        if isinstance(data, dict):
            return {**SSM_CONFIG_DEFAULT, **data}
    except (FileNotFoundError, json.JSONDecodeError, OSError):
        pass
    return dict(SSM_CONFIG_DEFAULT)


@app.route('/api/ssm/config', methods=['GET'])
def ssm_config_get():
    """Client settings for the SSM demo (currently just whether the
    Kinematics debug tab is shown — the dashboard's Demo Control tab
    toggles this)."""
    return jsonify(_load_ssm_config())


@app.route('/api/ssm/config', methods=['PUT'])
def ssm_config_put():
    data = request.get_json(force=True, silent=True) or {}
    with _ssm_config_lock:
        cfg = _load_ssm_config()
        if 'show_kinematics_tab' in data:
            cfg['show_kinematics_tab'] = bool(data['show_kinematics_tab'])
        try:
            with open(SSM_CONFIG_FILE, 'w', encoding='utf-8') as f:
                json.dump(cfg, f)
        except OSError as e:
            return jsonify({'error': f'could not save: {e}'}), 500
    return jsonify(cfg)
...
def pc_reference_angles():
    ...
    # Only the lazy first-time load needs the lock (same reasoning as
    # predict_pc's) — bones/compute_joint_angles below just read already-
    # built, never-mutated reference data, so release the lock before that
    # instead of holding it through jsonify's serialization too.
    with _pc_lock:
        try:
            model = _get_pc_model()
        except Exception as e:
...
@app.route('/api/pc_mean_bones')
def pc_mean_bones():
    """The mean-shape bones.json payload, served straight from the same
    in-memory reference every prediction (replay_shape, the PC-client
    export, /api/pc_reference_angles) already uses — the same object, not a
    separately-solved copy. The static bones.json file on disk is still
    written as a side effect of building this reference (see
    generate_isb_joints.process_and_export's export_path), but the app
    fetches it from here instead so there is exactly one source of truth
    for the mean pose and no way for the two to drift apart again (which is
    what happened when bones.json was regenerated by re-running the CLI
    script directly, outside the server's own lifecycle, and its FABRIK
    Step 4 search — not fully deterministic — landed on a different
    solution than the server's cached one)."""
    # Narrow lock scope, same reasoning as /api/pc_reference_angles just
    # above — every tablet loading the SSM demo hits this route once, so
    # holding the lock through JSON serialization here would needlessly
    # serialize concurrent page loads behind each other.
    with _pc_lock:
        try:
            model = _get_pc_model()
        except Exception as e:
            return jsonify({'error': str(e)}), 500
    return jsonify(model['reference']['payload'])
